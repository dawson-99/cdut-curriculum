...
void scan(sqQueue &Q) {
    ...
    for (int i = 0; i < MAXQSIZE; ++i) {
        if (Q.base[i].staus == 1 && Q.base[i].time < min && mainSize >= Q.base[i].needSize && Q.count < 2 && mainTapes >= Q.base[i].needTapes) {
            min = Q.base[i].time;
            positon1 = i;
        }
    }
...
//                Q.base[i].staus = 3;
        ...
        if (Q.base[position1].time < Q.base[position2].time) {
            ...
            mainSize += Q.base[position1].needSize;
            mainTapes += Q.base[position1].needTapes;
            ...
            mainSize += Q.base[position2].needSize;
            mainTapes += Q.base[position2].needTapes;
